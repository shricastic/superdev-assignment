...
pub async fn send_sol(req: web::Json<SolTransferReq>) -> impl Responder {
    let from_pubkey = match Pubkey::from_str(&req.from) {
        Ok(pk) => pk,
        Err(_) => return HttpResponse::BadRequest().json(ApiResponse::<()>::Error { success: false, error: "Invalid sender public key".into(), })
    };


    let to_pubkey = match Pubkey::from_str(&req.to) {
        Ok(pk) => pk,
        Err(_) => return HttpResponse::BadRequest().json(ApiResponse::<()>::Error { success: false, error: "Invalid receiver public key".into(), }) 
    };

    if req.lamports == 0 {
        return HttpResponse::BadRequest().json(ApiResponse::<()>::Error { success: false, error: "Amount must be greater than 0".into() })
    }
